...
trait Validate
{
  ...
  {
    // Sanitizar el input
    $input = trim($input); // Eliminar espacios al inicio y final
    ...
    if (is_numeric($input)) {
      return (float)$input; // Convertir a número y devolver
    } else {
      return 0; // Si no es número, devolver 0
    }
  }

  static function calculatePriceWithoutIVA($price,$iva) {
    // Sanitizar el precio
    $price = trim($price); // Eliminar espacios al inicio y final
    $price = htmlspecialchars($price, ENT_QUOTES, 'UTF-8'); // Convertir caracteres especiales en entidades HTML
    
    // Verificar si el precio es un número
    if (is_numeric($price)) {
      $priceWithoutIVA = $price / $iva; // Calcula el precio sin IVA
      return round($priceWithoutIVA, 2); // Redondear a 2 decimales y devolver
    } else {
      return 0; // Si no es número, devolver 0
    }
